...
Channel
	...
	.map {	row-> tuple(row.Sequence, row.Moniker, row.Filter, row.MS2path)}
	.set {	sample_run_ch	}
...
process runjob {
	...
	input:
	tuple val(sequence_file), val(experiment_name), val(filter_path), val(ms2_path)
	...
	pcpfm MS2_annotate -i $experiment_name -em HMDB_LMSD_annotated_preferred -nm MoNA_HMDB_LMSD_annotated_preferred --ms2_dir=$ms2_path
    ...
    pcpfm MS2_annotate -i $experiment_name -em HMDB_LMSD_annotated_masked_preferred_unknowns -nm MoNA_HMDB_LMSD_annotated_masked_preferred_unknowns --ms2_dir=$ms2_path

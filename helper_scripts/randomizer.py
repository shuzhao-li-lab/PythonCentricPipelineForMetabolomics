...
for b_no, batch in enumerate(split_samples(samples, NUM_BATCHES, None)):
    smallest_subgroup = min(get_value_counts(gen_expected(batch, NUM_BATCHES)))
    for sub_b_no, sub_batch in enumerate(split_samples(batch, int(round(smallest_subgroup)), None)):
